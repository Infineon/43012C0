...
sub post_process_cgs
{
    ...
    if($direct_load) {
        ...
        foreach my $line (@lines) {
            # 43012 patch dies if all "Data" entries are DIRECT_LOAD, so just convert code and data sections
            $line =~ s/^ENTRY \"Data\"/DIRECT_LOAD/
                    if $cgs_record->{file} !~ /43012C0/ ||
                      $line =~ /(CODE_AREA|SRAM_AREA|OVERFLOW_PATCH_CODE|INIT_PATCH_TABLE|INIT_AREA)/;
